...
def sign_up():
    ...
    if form.validate_on_submit():
        ...
        phone_number = phone_number.data if phone_number else request.form.get('phone_number', '').strip()
        
        id_number = getattr(form, 'id_number', None)
        id_number = id_number.data if id_number else request.form.get('id_number', '').strip()
        ...
        if password1 == password2:
            ...
            if id_number:
                ...
                if existing_id:
                    flash('Account not created! This ID number is already registered.', category='error')
                    return render_template('signup.html', form=form)
...
def forgot_password():
    ...
    if form.validate_on_submit():
        ...
        if (user and user.customer_profile and 
            user.customer_profile.phone_number == form.phone_number.data and 
            user.customer_profile.id_number == form.id_number.data):
            ...
            flash('Verification failed. Please check your Email, Phone Number, and ID Number.', 'error')
...
def api_sign_up():
    ...
    if not email or not first_name or not last_name or not password1 or not password2:
        return jsonify({'error': 'Email, first_name, last_name, password1, and password2 are required'}), 400
